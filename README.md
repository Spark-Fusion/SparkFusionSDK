...
- ✅ **自定义配置**：支持自定义应用名称、按钮文本等
- ✅ **数据存储**：基于 MMKV 的高性能数据存储
- ✅ **接口封装**：清晰的接口设计，实现细节不对外暴露
...
class MainActivity : AppCompatActivity() {
    ...
    private fun savePrivacyAgreementStatus(agreed: Boolean) {
        // 使用 SharedPreferences 或 MMKV 保存用户同意状态
        // ...
    }
...
- **不可取消**：对话框设置为不可取消（点击外部和返回键都不会关闭）
- **文本高亮**：自动高亮《隐私政策》文本，颜色为 `#33aaff`
- **点击跳转**：点击高亮的《隐私政策》文本会触发 `onClickWeb` 回调
- **自定义内容**：对话框内容会根据应用名称自动生成

## 注意事项

1. **初始化时机**：必须在 Application 的 `onCreate()` 中初始化 SDK
2. **Context 使用**：`showPrivacyPolicyDialog` 需要 Activity Context，不能使用 Application Context
3. **对话框行为**：对话框默认不可取消，用户必须选择"同意"或"拒绝"
4. **存储依赖**：SDK 使用 MMKV 进行数据存储，确保已正确初始化
